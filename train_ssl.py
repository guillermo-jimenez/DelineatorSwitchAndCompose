...
def main(config_file, model_name, input_files, bool_hpc):
    ...
    shutil.copyfile("./train_ssl.py",os.path.join(target_path,model_name,"train_ssl.py"))
    shutil.copyfile("./src/loss.py",os.path.join(target_path,model_name,"src_loss.py"))
    shutil.copyfile("./src/data.py",os.path.join(target_path,model_name,"src_data.py"))
    shutil.copyfile("./src/train.py",os.path.join(target_path,model_name,"src_train.py"))
    shutil.copyfile("./src/metrics.py",os.path.join(target_path,model_name,"src_metrics.py"))
    ...
    for i,(ix_train,ix_valid) in enumerate(zip(indices_train,indices_valid)):
        ...
        # Define models
        if "teacher_weights" in execution:
            if isinstance(execution["teacher_weights"], list):
                model_teacher = torch.load(execution["teacher_weights"][i],pickle_module=dill)
            else:
                model_teacher = torch.load(execution["teacher_weights"],pickle_module=dill)
        else:
            model_teacher = sak.from_dict(execution["model"]).float().cuda()
        if "student_weights" in execution:
            if isinstance(execution["student_weights"], list):
                model_student = torch.load(execution["student_weights"][i],pickle_module=dill)
            else:
                model_student = torch.load(execution["student_weights"],pickle_module=dill)
        else:
        ...
        src.train.train_valid_model_ssl(model_student,model_teacher,state,execution,loader_train,loader_mpl,loader_valid)
